...
//   norm  +S    alpha +S    User  +S    PSKst PSKnn    
    ...
    {0xCA, 0xCA, 0x10, 0x10, 0xCA, 0xCA, 0x10, 0x10},         // 18 sk_Exp            x e^x     A   a
    {0xC0, 0xC0, 0x30, 0x30, 0xC0, 0xC0, 0x30, 0xC0},         // 19 sk_Ln               LN      B   b
    {0x80, 0x80, 0x70, 0x70, 0x80, 0x80, 0x70, 0x80},         // 20 sk_Log              LOG     C   c
    {0x30, 0x30, 0x80, 0x80, 0x30, 0x30, 0x80, 0x30},         // 21 sk_Imaginary        1/x     D   d
    {0x10, 0x10, 0xC0, 0xC0, 0x10, 0x10, 0xC0, 0x10},         // 22 sk_Comma            Sigma+  E   e
    {0x39, 0x39, 0x11, 0x73, 0x39, 0x39, 0x11, 0x00},         // 23 sk_Power          x y^x     F   °
    {0x71, 0x71, 0x31, 0x71, 0x71, 0x71, 0x31, 0x71},         // 24 sk_Sine             SIN     G   !=
    {0x81, 0x81, 0x71, 0x81, 0x81, 0x81, 0x71, 0x81},         // 25 sk_Cosine           COS     H   <
    {0xC1, 0xC1, 0x81, 0xC1, 0xC1, 0xC1, 0x81, 0xC1},         // 26 sk_Tangent          TAN     I   >
    {0x73, 0x73, 0xC1, 0x71, 0x73, 0x73, 0xC1, 0x73},         // 27 sk_Pi               CHS     J   !=
    {0x70, 0x70, 0x32, 0x81, 0x70, 0x70, 0x32, 0x70},         // 28 sk_Sqrt             SQRT    K   <
...
void kbdDown() {
    ...
    if (kdown == 0) {           // no key
        if (stack[0] == 0x0DFA) {           // pkseq ST
            ...
            j = 6;
        } else if (stack[0] == 0x0CED) {   // __ first digit
            j = 7;
